...
module.exports.deploy = (options, callback) => {
    return setup(options)
        ...
        .then(() => {
            console.log(
                chalk.green.bold(`Deployed ${config.publicRoot} to ${config.bucket} on S3!`)
            );
            if (callback) {
...
function getFiles() {
    return new Promise((resolve, reject) => {
        new Glob('**/*.*', { cwd: config.publicRoot, dot: true }, (err, files) => {
            ...
            const addHeaders =
                Array.isArray(config.putObjectParams) && config.putObjectParams.length > 0;
...
function uploadFiles(files) {
    ...
    const processFile = (file, callback) => {
        checkIfUploadRequired(file, (err, required) => {
            if (err) {
                console.log(`Error checking ${file.path.dir}/${file.path.base} : ${err}`);
                return callback(err);
            }
            if (required) {
